...
def writeFilterRules(filePath):
    with open(filePath, "r") as file:
        ...
        with open("createRulesInIptables.sh", "w") as outfile:
            ...
            outfile.write("iptables -t filter -N INTERNETRULES \n")
            outfile.write("iptables -t filter -N SUBNETS \n")
            outfile.write("iptables -t filter -F RULES \n")
            outfile.write("iptables -t filter -F INTERNETRULES \n")
            ...
            for line in lines:
                ...
                if temp[0].strip() == "Subnet":
                    ...
                    outfile.write("iptables -t filter -A SUBNETS -s {} -j INTERNETRULES \n".format(subnet))
                    outfile.write("iptables -t filter -A SUBNETS -d {} -j INTERNETRULES \n".format(subnet))
                    continue
                else:                 
                    source  = temp[0].strip()
                    ports   = temp[1].strip()
                ...
                if (source == "internet") or (destine == "internet"):
                    rule = "iptables -t filter -A INTERNETRULES "
                else:
                ...
                if (destine != "*") and (destine != "internet"):
            ...
            #outfile.write("iptables -t filter -A SUBNETS -j INTERNETRULES \n")
            outfile.write("iptables -t filter -A INTERNETRULES -j DROP \n")
